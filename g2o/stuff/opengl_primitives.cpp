...
class GLUWrapper
{
  ...
  protected:
    GLUWrapper()
    {
        ...
        gluQuadricNormals(_quadratic,
                          GLU_SMOOTH); // Create Smooth Normals ( NEW )
    }
    ~GLUWrapper()
    {
        // std::cerr << __PRETTY_FUNCTION__ << std::endl;
...
void drawRangeRing(GLfloat range, GLfloat fov, GLfloat range_width)
{
    ...
    gluPartialDisk(GLUWrapper::getQuadradic(), range, range + range_width, 32,
                   1, 0.f, fov);
    glPopMatrix();
}
               ...
               int slices_per_circle)
{
    ...
    glPushMatrix();
    glBegin(GL_TRIANGLES);
    glNormal3f((float)std::sin(-fov_rad * 0.5),
