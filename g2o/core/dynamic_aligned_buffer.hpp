...
template <typename Type> Type *allocate_aligned(size_t n)
{
...
template <typename Type> void free_aligned(Type *block)
{
    Eigen::internal::aligned_free(block);
}

template <typename Type> struct dynamic_aligned_buffer
{
    dynamic_aligned_buffer(size_t size) : m_size{0}, m_ptr{nullptr}
    {
        allocate(size);
    }
