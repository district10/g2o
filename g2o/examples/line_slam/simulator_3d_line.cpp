...
{
    return Vector4d(gauss_rand(cov(0)), gauss_rand(cov(1)), gauss_rand(cov(2)),
                    gauss_rand(cov(3)));
}
...
struct Robot : public WorldItem
{
    ...
    void move(const Isometry3d &newPosition, int &id)
    {
        ...
        graph()->addVertex(v);
        if (_planarMotion) {
            // add a singleton constraint that locks the position of the robot
            // on the plane
...
struct Simulator : public SimulatorItem
{
    ...
    void sense(int robotIndex)
    {
        ...
        for (WorldItemSet::iterator it = _world.begin(); it != _world.end();
             ++it) {
            WorldItem *item = *it;
            r->sense(item);
        }
...
int main(int argc, char **argv)
{
    ...
    if (planarMotion) {
        ...
        iq =
            Quaterniond(AngleAxisd(-0.2, Vector3d::UnitZ()).toRotationMatrix());
    } else {
