...
// HACK force tictoc statistics
        ...
        // adding edges to the graph. Add all edges connecting a node and then
        // call optimize
        tictoc("inc_optimize");
        bool freshlySolved = false;
        int lastNode = 2;
        slamInterface.addNode("", 0, graphDimension, vector<double>());
        for (vector<EdgeInformation>::const_iterator it =
                 edgesFromGraph.begin();
            ...
            if (minNodeId > lastNode) {
                ...
                lastNode = minNodeId;
                freshlySolved = true;
                G2oSlamInterface::SolveResult solverState =
                    slamInterface.solve();
                if (!verbose) {
                    switch (solverState) {
