                                  ...
                                  public LinearSolverCholmodOnlineInterface
{
  public:
    ...
    bool solve(const SparseBlockMatrix<MatrixType> &A, double *x, double *b)
    {
        ...
        if (_cholmodCommon.status == CHOLMOD_NOT_POSDEF) {
            std::cerr << "solve(): Cholesky failure, writing debug.txt "
                         "(Hessian loadable by Octave)"
                      << std::endl;
  ...
  public:
    void computeSymbolicDecomposition(const SparseBlockMatrix<MatrixType> &A)
    {
        ...
        int amdStatus = camd_order(
            _matrixStructure.n, _matrixStructure.Ap, _matrixStructure.Aii,
            _blockPermutation.data(), NULL, NULL, cmember->data());
        if (amdStatus != CAMD_OK) {
            std::cerr << "Error while computing ordering" << std::endl;
        }

        // blow up the permutation to the scalar matrix and extend to include
        ...
        _cholmodFactor =
            cholmod_analyze_p(_cholmodSparse, _scalarPermutation.data(), NULL,
                              0, &_cholmodCommon);
