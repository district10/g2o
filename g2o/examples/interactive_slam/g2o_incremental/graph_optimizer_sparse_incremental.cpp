...
template <int p, int l> std::unique_ptr<g2o::Solver> AllocateCholmodSolver()
{
...
SparseOptimizerIncremental::SparseOptimizerIncremental()
{
    ...
    _permutedUpdate = cholmod_allocate_triplet(1000, 1000, 1024, 0,
                                               CHOLMOD_REAL, &_cholmodCommon);
    _L = 0;
    _cholmodFactor = 0;
    _solverInterface = 0;
    ...
    if (!online || batchStep) {
        ...
        if (slamDimension == 3) {
            for (size_t i = 0; i < indexMapping().size(); ++i) {
                OnlineVertexSE2 *v =
                    static_cast<OnlineVertexSE2 *>(indexMapping()[i]);
                v->setEstimate(v->updatedEstimate);
            }
        } else if (slamDimension == 6) {
            for (size_t i = 0; i < indexMapping().size(); ++i) {
                OnlineVertexSE3 *v =
                    static_cast<OnlineVertexSE3 *>(indexMapping()[i]);
                v->setEstimate(v->updatedEstimate);
            }
        }
...
// backup the tempindex and prepare sorting structure
    ...
    sort(backupIdx, backupIdx + _touchedVertices.size()); // sort according to
                                                          // the hessianIndex
                                                          // which is the same
                                                          // order as used later
                                                          // by the optimizer
    ...
    for (HyperGraph::EdgeSet::const_iterator it = eset.begin();
        ...
        OptimizableGraph::Edge *e = static_cast<OptimizableGraph::Edge *>(*it);
        OptimizableGraph::Vertex *v1 =
            (OptimizableGraph::Vertex *)e->vertices()[0];
        OptimizableGraph::Vertex *v2 =
            (OptimizableGraph::Vertex *)e->vertices()[1];
...
bool SparseOptimizerIncremental::computeCholeskyUpdate()
{
    ...
    if (_cholmodCommon.status == CHOLMOD_NOT_POSDEF) {
        // std::cerr << "Cholesky failure, writing debug.txt (Hessian loadable
        // by Octave)" << std::endl;
        // writeCCSMatrix("debug.txt", _cholmodSparse->nrow,
        // _cholmodSparse->ncol, (int*)_cholmodSparse->p,
...
bool SparseOptimizerIncremental::initSolver(int dimension, int batchEveryN)
{
    ...
    if (dimension == 3) {
        ...
        LinearSolverCholmodOnline<Matrix3d> *s =
            dynamic_cast<LinearSolverCholmodOnline<Matrix3d> *>(
                &bs->linearSolver());
