...
MainWindow::MainWindow(QWidget *parent, Qt::WindowFlags flags)
    : QMainWindow(parent, flags), _lastSolver(-1), _currentSolver(0),
      _viewerPropertiesWidget(0), _optimizerPropertiesWidget(0), _filename("")
{
    setupUi(this);
    leKernelWidth->setValidator(
        new QDoubleValidator(-numeric_limits<double>::max(),
                             numeric_limits<double>::max(), 7, this));
...
void MainWindow::setRobustKernel()
{
    ...
    if (robustKernel) {
        ...
        AbstractRobustKernelCreator *creator =
            RobustKernelFactory::instance()->creator(
                strRobustKernel.toStdString());
        if (!creator) {
            cerr << strRobustKernel.toStdString()
