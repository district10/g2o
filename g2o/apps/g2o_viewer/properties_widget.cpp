...
PropertiesWidget::PropertiesWidget(QWidget *parent, Qt::WindowFlags f)
    : QDialog(parent, f), _properties(0)
{
    setupUi(this);
}
...
void PropertiesWidget::updateDisplayedProperties()
{
    ...
    for (PropertyMap::PropertyMapIterator it = properties->begin();
        ...
        textItem->setText(
            QString::fromStdString(humanReadablePropName(it->first)));
...
void PropertiesWidget::applyProperties()
{
    ...
    for (int r = 0; r < tableWidget->rowCount(); ++r) {
        ...
        if (dynamic_cast<Property<bool> *>(baseProp)) {
            ...
            if (!status) {
                cerr << "Warning: unable to set property " << baseProp->name()
                     << endl;
            }
        }
    }
...
std::string
PropertiesWidget::humanReadablePropName(const std::string &propertyName) const
{
    return propertyName;
}
